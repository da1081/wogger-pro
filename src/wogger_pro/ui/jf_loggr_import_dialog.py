"""Dialog for importing entries from JF LoggR JSON exports."""
...
from ..core.importer import ImportValidationError, MergeResult, merge_entries, parse_jf_loggr_json
from ..core.models import Entry
from ..core.prompt_manager import PromptManager
from ..core.repository import EntriesRepository
from ..core.paths import default_downloads_dir

LOGGER = logging.getLogger("wogger.ui.import.jf_loggr")


@dataclass(slots=True)
class JfLoggrImportSummary:
    file_path: Path
...
class JfLoggrImportDialog(QDialog):
    def __init__(
        self,
        ...
        self.setWindowTitle("Import JF LoggR Data")
        self.setModal(True)
        self._repository = repository
        self._prompt_manager = prompt_manager
        self._summary: JfLoggrImportSummary | None = None

        self._build_ui()

    @property
    def summary(self) -> JfLoggrImportSummary | None:
    ...
    def _build_ui(self) -> None:
        ...
        intro = QLabel(
            "Select a JF LoggR export file (typically named worklogger.json). "
            "Entries will be converted to Wogger Pro segments.",
            self,
        )
        ...
        self._path_edit.setPlaceholderText("Select a JF LoggR JSON file…")
    ...
    def _on_browse(self) -> None:
        start_dir = str(default_downloads_dir())
        file_path, _ = QFileDialog.getOpenFileName(
            self,
            "Select JF LoggR export",
            start_dir,
            "JF LoggR export (worklogger.json);;JSON Files (*.json);;All Files (*.*)",
        )
        if file_path:
    ...
    def _on_accept(self) -> None:
        ...
        if not path_text:
            self._status_label.setText("Please select a JF LoggR JSON file to import.")
            return

        json_path = Path(path_text).expanduser()

        try:
            imported_entries = parse_jf_loggr_json(json_path)
        ...
        self._summary = JfLoggrImportSummary(
            file_path=json_path,
        ...
        LOGGER.info(
            "JF LoggR import completed",
            extra={
                "event": "ui_import_jf_loggr",
                "file": json_path.name,
                "source_entries": len(imported_entries),
    ...
    def _build_summary_text(
        ...
        lines: list[str] = [
            ...
            f"Entries in file: {len(imported_entries)} ({source_minutes} minute{'s' if source_minutes != 1 else ''})",
        ...
        if applied_count:
            if applied_start and applied_end:
                ...
                lines.append("Adjusted import span: (calculated from applied segments)")
            lines.append(
        ...
        if prefer_imported:
            ...
            lines.append("Strategy: overwrite conflicts with imported data.")
            lines.append(
                "Overlapping portions of existing entries will be trimmed or replaced so that the JF LoggR data takes precedence."
            )
        else:
            ...
            lines.append("Strategy: keep existing entries and fit imports into free time.")
            lines.append(
                "Only the portions of the JF LoggR export that do not overlap existing entries will be added; overlapping minutes are skipped."
            )
    ...
    def _format_completion_message(self, summary: JfLoggrImportSummary) -> str:
        lines: list[str] = []
        ...
        if summary.applied_entry_count:
            ...
            lines.append(
                f"No new time was imported from {summary.file_path.name} because existing entries already cover the exported data."
            )
